...
export const searchWeb = cache(
  async (query: string | null, count: number = 5): Promise<any[]> => {
    console.log(
      `[searchWeb] Starting search with query: "${query}", count: ${count}`
    );
    ...
    const fetchWithRetry = async (retryCount = 0): Promise<any[]> => {
      ...
      try {
        console.log(
          "[fetchWithRetry] [WEB] Sending request to Brave Search API"
        );
        ...
        console.log(
          `[fetchWithRetry] [WEB] Received response with status: ${response.status}`
        );
        ...
        if (!response.ok) {
          if (response.status === 429 && retryCount < 3) {
            ...
            console.warn(
              `[fetchWithRetry] [WEB] Rate limit hit. Retrying in ${delay}ms`
            );
        ...
        console.log(
          `[fetchWithRetry] [WEB] Retrieved ${results.length} results`
        );

        return results;
      } catch (error) {
        console.error(
          "[fetchWithRetry] [WEB] Error fetching data from Brave Search API:",
          error
        );
        return [];
      }
    ...
    console.log(
      `[searchWeb] Search completed. Returning ${results.length} results`
    );
    return results;
  }
);
